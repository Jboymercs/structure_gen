...
public class EntityTimedAttackShield <T extends EntityEnderShield & IAttack> extends EntityAIBase {
    ...
    public EntityTimedAttackShield(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityTimedAttackShield(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if (distSq <= 2 && canSee) {
            ...
            ++this.strafingTime;
        } else if(!entity.isRandomGetAway){
