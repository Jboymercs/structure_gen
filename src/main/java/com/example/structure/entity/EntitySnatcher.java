...
public class EntitySnatcher extends EntityModBase implements IAttack, IAnimatable {
    ...
    public void initEntityAI() {
        super.initEntityAI();
        this.tasks.addTask(3, new EntityStalkAI<>(this, 1.0, 20, 20, 2, 0.3f, 10, true));
   ...
   addEvent(()-> {
    ...
    protected int hibernationTimer = 500 + ModRand.range(50, 1000);
    ...
    public void onUpdate() {
        ...
        if(this.isCurrentlyinHibernation) {
            ...
            List<EntityPlayer> closeplayers = this.world.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().grow(2D), e -> !e.getIsInvulnerable());
            ...
            if(!closeplayers.isEmpty() && !this.setOnaggro) {
                this.endHiberntaion();
                hibernationTimer = 500 + ModRand.range(50, 1000);
    ...
    private <E extends IAnimatable> PlayState predicateArms(AnimationEvent<E> event) {
        ...
        if (!(event.getLimbSwingAmount() > -0.10F && event.getLimbSwingAmount() < 0.10F) && !this.isSpotted() && !this.isDigDown() && !this.isAttacking()) {
    ...
    public int startAttack(EntityLivingBase target, float distanceSq, boolean strafingBackwards) {
        ...
        if(!this.isAttacking()) {
            List<Consumer<EntityLivingBase>> attacks = new ArrayList<>(Arrays.asList(basic_attack));
            double[] weights = {
    ...
    private Consumer<EntityLivingBase> basic_attack = (target)-> {
      ...
      addEvent(()-> this.setAttacking(false), 25);
    };
